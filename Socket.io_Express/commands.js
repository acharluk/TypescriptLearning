...
module.exports = {
    ...
    'specs': { description: "Gives server specs", run() { return `${os.cpus().length}x ${os.cpus()[0].model} | RAM: Total=${Math.floor(os.totalmem()/(1024*1024))}MB Free=${Math.floor(os.freemem()/(1024*1024))}MB | OS: ${os.type()} ${os.platform()} ${os.arch()} ${os.release()}` } }
}